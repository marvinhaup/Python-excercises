# Programm, dass Daten aus einer Datei ausließt und entsprechend der Struktur eines Objekte abspeichert
...
fid = open('Dateien\Sortiment.txt','r')
content = fid.readlines()
contentLength = int(len(content)/5)
myList = []

for i in range(contentLength):
    brand = content[i*5]
    model = content[i*5+1]
    prodYear = content[i*5+2]
    price = content[i*5+3]
    myList += [Auto(brand,model,prodYear,price)]

for tup in myList:
    print("Auto:")
    print(tup.getHersteller())
    print(tup.getModell())
    print(tup.getProduktionsjahr())
    print(tup.getPreis())
fid.close()